...
export function HyperText({
  ...
  const textWidthRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (elementRef.current && !textWidthRef.current) {
      const computedWidth = window.getComputedStyle(elementRef.current).width;
      textWidthRef.current = computedWidth;
  ...
  return (
    <MotionComponent
      ref={elementRef}
      className={cn('py-2 text-4xl overflow-visible', className)}
      onMouseEnter={handleAnimationTrigger}
      style={{ width: textWidthRef.current || 'auto' }}
      {...props}
    >
      <AnimatePresence>
