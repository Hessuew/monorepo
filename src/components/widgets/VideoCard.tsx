...
export const VideoCard: FC<VideoCardProps> = ({ videoSrc, thumbnailSrc, title, description, iconName }) => {
  ...
  return (
    <div className='w-full rounded-3xl bg-transparent'>
      ...
      >
        {/* Video element */}
        <video ref={videoRef} src={videoSrc} muted playsInline loop className='rounded-3xl w-full' />

        {/* Thumbnail overlay */}
        <div className={`absolute inset-0 transition-opacity duration-300 ${isHovering ? 'opacity-0' : 'opacity-100'}`}>
          <img src={thumbnailSrc} alt={`Thumbnail for ${title}`} className='w-full h-full object-cover rounded-3xl' />
        </div>
      </div>
